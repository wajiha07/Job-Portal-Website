...
class WP_Job_Board_Pro_Widget_Candidate_Filter extends WP_Widget {
	/**
	 * Initialize widget
	 *
	 * @access public
	...
	function __construct() {
		parent::__construct(
			...
			__( 'Candidate Filter', 'wp-job-board-pro' ),
			array(
				'description' => __( 'Filter for filtering candidates.', 'wp-job-board-pro' ),
			)
		);
	}
	...
	function widget( $args, $instance ) {
		include WP_Job_Board_Pro_Template_Loader::locate( 'widgets/candidate-filter' );
	}
	...
	function form( $instance ) {
		...
		<ul class="wp-job-board-pro-filter-fields wp-job-board-pro-filter-candidate-fields">
			<?php

			$all_fields = $fields = WP_Job_Board_Pro_Candidate_Filter::get_fields();
			if ( ! empty( $sort ) ) {
		...
		<script type="text/javascript">
			jQuery(document).ready(function($) {
				$('.widget .wp-job-board-pro-filter-candidate-fields').each(function() {
					var el = $(this);
...
register_widget('WP_Job_Board_Pro_Widget_Candidate_Filter');