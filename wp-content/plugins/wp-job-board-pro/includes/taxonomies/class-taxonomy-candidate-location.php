...
class WP_Job_Board_Pro_Taxonomy_Candidate_Location{
	...
	public static function definition() {
		...
		$rewrite_slug = get_option('wp_job_board_pro_candidate_location_slug');
		if ( empty($rewrite_slug) ) {
			$rewrite_slug = _x( 'candidate-location', 'Candidate location slug - resave permalinks after changing this', 'wp-job-board-pro' );
		}
		$rewrite = array(
			'slug'         => $rewrite_slug,
			'with_front'   => false,
		...
		register_taxonomy( 'candidate_location', 'candidate', array(
			'labels'            => apply_filters( 'wp_job_board_pro_taxomony_candidate_location_labels', $labels ),
			'hierarchical'      => true,
			'rewrite'           => $rewrite,
			'public'            => true,
			'show_ui'           => true,
...
WP_Job_Board_Pro_Taxonomy_Candidate_Location::init();