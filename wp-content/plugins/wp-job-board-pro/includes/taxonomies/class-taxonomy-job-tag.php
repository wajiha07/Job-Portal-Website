...
class WP_Job_Board_Pro_Taxonomy_Job_Tag{
	...
	public static function definition() {
		...
		$rewrite_slug = get_option('wp_job_board_pro_job_tag_slug');
		if ( empty($rewrite_slug) ) {
			$rewrite_slug = _x( 'job-tag', 'Job tag slug - resave permalinks after changing this', 'wp-job-board-pro' );
		}
		$rewrite = array(
			'slug'         => $rewrite_slug,
			'with_front'   => false,
		...
		register_taxonomy( 'job_listing_tag', 'job_listing', array(
			'labels'            => apply_filters( 'wp_job_board_pro_taxomony_job_tag_labels', $labels ),
			'hierarchical'      => false,
			'rewrite'           => $rewrite,
			'public'            => true,
			'show_ui'           => true,
...
WP_Job_Board_Pro_Taxonomy_Job_Tag::init();